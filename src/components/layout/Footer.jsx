...
const Footer = () => {
  ...
  return (
    <footer className="bg-brand-bg-primary text-brand-silver-light pt-24 pb-12 relative overflow-hidden border-t-2 border-brand-red">
      <div className="absolute inset-0 bg-gradient-to-b from-brand-bg-primary to-brand-bg-secondary opacity-50 z-0"></div>
      ...
      <div className="max-w-[1500px] mx-auto px-6 sm:px-8 lg:px-10 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 lg:gap-16 mb-16">
          ...
          <div className="md:col-span-5 flex flex-col justify-between">
            <div>
              <div className="bg-brand-bg-secondary p-4 rounded-2xl inline-block mb-6 shadow-lg border border-white/5">
                <img src="/logo.png" alt="Harmain Travels Logo" className="h-12 sm:h-16 object-contain" loading="lazy" 
                  onError={(e) => {
                    e.target.style.display = 'none';
                    e.target.parentElement.querySelector('.fallback-logo-footer').style.display = 'flex';
                  }}
                />
                <div className="fallback-logo-footer hidden flex-col items-center">
                  <span className="text-brand-red font-display font-black text-2xl uppercase tracking-widest">HT</span>
                  <span className="text-brand-silver-light font-display font-bold text-[10px] mt-1 uppercase">HARMAIN</span>
                </div>
              </div>
              <p className="text-brand-muted text-sm sm:text-base font-body tracking-wide leading-relaxed max-w-sm mb-6">
                Your premium travel partner. Curating elite corporate escapes, luxury umrah, and world-class itineraries.
              </p>
            </div>
          ...
          <div className="md:col-span-3">
            <span className="text-brand-silver font-display font-semibold uppercase tracking-[0.2em] text-[11px] sm:text-xs mb-6 block border-b border-white/10 pb-2">Navigation</span>
            <ul className="space-y-3.5">
              {footerLinks.map((link, index) => (
                <li key={index}>
                  <Link 
                    to={link.path} 
                    className="text-brand-silver-light hover:text-brand-red font-display font-medium text-sm transition-all duration-300 flex items-center gap-2 hover:translate-x-2"
                  >
          ...
          <div className="md:col-span-4">
            <span className="text-brand-silver font-display font-semibold uppercase tracking-[0.2em] text-[11px] sm:text-xs mb-6 block border-b border-white/10 pb-2">Get in Touch</span>
            <ul className="space-y-5">
              <li className="flex items-start gap-4">
                <div className="w-10 h-10 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-brand-red shrink-0">
                  <MapPin size={18} />
                </div>
                <div>
              ...
              <li className="flex items-center gap-4">
                ...
                <div>
                  <span className="text-brand-muted block text-[10px] font-display font-medium uppercase tracking-widest mb-1">Email Inquiries</span>
                  <a href="mailto:harmaintravelisb@gmail.com" className="text-brand-white font-body text-sm hover:text-brand-red transition-all">harmaintravelisb@gmail.com</a>
                </div>
              </li>
            </ul>
          </div>
        ...
        <div className="pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-6">
          <p className="text-brand-muted font-body text-[11px] sm:text-xs">
