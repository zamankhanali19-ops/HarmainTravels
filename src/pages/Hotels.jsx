      ...
      <section className="pt-64 pb-32 bg-[#002147] relative z-10">
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          <div className="text-center mb-20">
            <span className="text-[#A61D24] font-black uppercase tracking-[0.5em] text-xs mb-8 block">Iconic Stays</span>
          ...
          <div className="bg-white/95 backdrop-blur-xl border border-white/20 rounded-[2rem] sm:rounded-[2.5rem] md:rounded-[3.5rem] p-3 sm:p-4 xl:px-5 xl:py-4 max-w-7xl mx-auto shadow-2xl animate-reveal delay-200 relative z-30">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 items-stretch">
              ...
              <div className="text-left bg-slate-50/80 p-3 rounded-xl sm:rounded-2xl border border-slate-100 flex flex-col justify-between h-full min-h-[75px] hover:border-[#A61D24]/20 transition-colors relative">
                <label className="text-[8px] font-black uppercase tracking-wider text-slate-400 mb-1">Check-In</label>
                <button
                  type="button"
              ...
              {/* Check-out Date */}
              <div className="text-left bg-slate-50/80 p-3 rounded-xl sm:rounded-2xl border border-slate-100 flex flex-col justify-between h-full min-h-[75px] hover:border-[#A61D24]/20 transition-colors relative">
                <label className="text-[8px] font-black uppercase tracking-wider text-slate-400 mb-1">Check-Out</label>
                <button
                  type="button"
              ...
              <div className="text-left bg-slate-50/80 p-3 rounded-xl sm:rounded-2xl border border-slate-100 flex flex-col justify-between h-full min-h-[75px] hover:border-[#A61D24]/20 transition-colors relative">
                <label className="text-[8px] font-black uppercase tracking-wider text-slate-400 mb-1">Guests</label>
                <button
                  type="button"
...
- Guests: ${adults} Adults, ${children} Children, ${infants} Infants`;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
          {[
            ...
            <div key={i} className="flex gap-8 items-start">
              <div className="w-20 h-20 bg-slate-50 rounded-3xl flex items-center justify-center text-[#A61D24] shrink-0">
                {item.i}
              </div>
              <div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-24">
            <h2 className="text-5xl md:text-7xl font-black text-[#002147] uppercase tracking-tighter">
              Explore <span className="text-[#A61D24]">Iconic Stays</span>
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            {featuredHotels.map((hotel, i) => (
              <div key={i} className="group bg-white rounded-[2.5rem] md:rounded-[3rem] overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 cursor-pointer">
                <div className="relative h-48 md:h-72 overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 md:top-6 md:left-6 px-3 md:px-4 py-1 md:py-2 bg-white/90 backdrop-blur-md rounded-full text-[10px] font-black uppercase tracking-widest text-[#002147]">
                    {hotel.tag}
                  </div>
                  <div className="absolute bottom-4 right-4 md:bottom-6 md:right-6 px-3 md:px-4 py-1 md:py-2 bg-[#A61D24] rounded-full text-[10px] font-black uppercase tracking-widest text-white">
                    {hotel.price}
                  </div>
                </div>
                <div className="p-5 md:p-8">
                  <div className="flex justify-between items-start mb-4 md:mb-6 gap-2">
                    <div className="min-w-0 flex-1">
                      <h3 className="text-lg md:text-xl font-black text-[#002147] uppercase tracking-tighter mb-2 break-words leading-tight line-clamp-1">{hotel.name}</h3>
                      <p className="text-slate-400 font-bold text-xs uppercase tracking-widest flex items-center gap-2 truncate">
                        <MapPin size={12} className="text-[#A61D24] shrink-0" /> {hotel.location}
                      </p>
                    </div>
                    <div className="flex text-gold shrink-0">
                      {[...Array(hotel.rating)].map((_, i) => <Star key={i} size={12} className="md:w-[14px]" fill="#C19A5B" color="#C19A5B" />)}
                    </div>
                  </div>
                  <a 
                    href={`https://wa.me/923175477919?text=I am interested in booking a stay at ${hotel.name} in ${hotel.location}.`}
                    target="_blank"
                    className="flex items-center justify-between w-full p-3.5 border-2 border-slate-50 rounded-2xl group-hover:bg-[#002147] group-hover:border-[#002147] group-hover:text-white transition-all duration-500"
                  >
      ...
      <section className="py-40 bg-white">
        <div className="max-w-5xl mx-auto px-4 bg-[#002147] rounded-[4rem] p-16 md:p-24 text-center text-white relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-[#A61D24]/20 rounded-full blur-3xl"></div>
          <h2 className="text-4xl md:text-6xl font-black uppercase tracking-tighter mb-8 relative z-10">
            Looking for something <span className="text-[#A61D24]">Bespoke</span>?
          </h2>
